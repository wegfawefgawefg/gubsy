...
void render_menu(int width, int height) {
    ...
    auto draw_text_scaled = [&](const char* s, int x, int y, SDL_Color col, float scale) {
        ...
        if (SDL_Surface* surf = TTF_RenderUTF8_Blended(gg->ui_font, s, col)) {
            ...
            int sw = static_cast<int>(std::lround(static_cast<double>(tw) * scale_d));
            int sh = static_cast<int>(std::lround(static_cast<double>(th) * scale_d));
            SDL_Rect td{x, y, sw, sh}; SDL_RenderCopy(r, tex, nullptr, &td);
            SDL_DestroyTexture(tex); SDL_FreeSurface(surf);
        }
    };
