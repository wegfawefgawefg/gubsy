...
std::vector<LayoutItem> lobby_layout_items(const std::vector<ButtonDesc>& buttons, int width, int height) {
    ...
    items.reserve(buttons.size() + 1);
    for (const auto& b : buttons) {
        if (const char* layout_id = layout_id_for_button(b.id)) {
            items.push_back(LayoutItem{layout_id, to_pixels(b.rect, width, height)});
        }
    }
    items.push_back(LayoutItem{"mods_panel", to_pixels(layout_rect("mods_panel", kModsPanelFallback), width, height)});
    return items;
}

bool lobby_layout_edit_enabled() {
    return layout_edit_enabled();
}
...
void lobby_layout_editor_handle(const std::vector<ButtonDesc>& buttons, int width, int height) {
    layout_handle_editor(lobby_layout_items(buttons, width, height), width, height);
}

void render_lobby_layout_overlay(SDL_Renderer* r, const std::vector<ButtonDesc>& buttons, int width, int height) {
    layout_render_overlay(r, lobby_layout_items(buttons, width, height), width, height);
}
...
std::vector<ButtonDesc> build_lobby_buttons() {
    ...
    buttons.push_back(ButtonDesc{1400, layout_rect("lobby_name", row_rect(0)), "Lobby Name", ButtonKind::Button, 0.0f, true});
    buttons.push_back(ButtonDesc{1401, layout_rect("privacy", row_rect(1)), "Privacy", ButtonKind::OptionCycle, 0.0f, true});
    buttons.push_back(ButtonDesc{1402, layout_rect("invite_friends", row_rect(2)), "Invite Friends", ButtonKind::Button, 0.0f, false});
    buttons.push_back(ButtonDesc{1403, layout_rect("scenario", row_rect(3)), "Scenario", ButtonKind::OptionCycle, 0.0f, true});
    buttons.push_back(ButtonDesc{1404, layout_rect("difficulty", row_rect(4)), "Difficulty", ButtonKind::OptionCycle, 0.0f, true});
    buttons.push_back(ButtonDesc{1405, layout_rect("max_players", row_rect(5)), "Max Players", ButtonKind::OptionCycle, 0.0f, true});
    buttons.push_back(ButtonDesc{1406, layout_rect("allow_join", row_rect(6)), "Allow Join", ButtonKind::Toggle, lobby_const().allow_join ? 1.0f : 0.0f, true});
    buttons.push_back(ButtonDesc{1407, layout_rect("seed", row_rect(7)), "Seed", ButtonKind::Button, 0.0f, true});
    buttons.push_back(ButtonDesc{1408, layout_rect("randomize_seed", RectNdc{0.06f, 0.72f, 0.20f, 0.065f}), "Randomize Seed", ButtonKind::Button, 0.0f, true});
    buttons.push_back(ButtonDesc{1409, layout_rect("manage_mods", RectNdc{0.52f, 0.70f, 0.36f, 0.07f}), "Manage Mods", ButtonKind::Button, 0.0f, true});
    buttons.push_back(ButtonDesc{1440, layout_rect("start_game", RectNdc{0.56f, 0.85f, 0.32f, 0.08f}), "Start Game", ButtonKind::Button, 0.0f, true});
    buttons.push_back(ButtonDesc{1441, layout_rect("back", RectNdc{0.06f, 0.82f, 0.22f, 0.07f}), "Back", ButtonKind::Button, 0.0f, true});
    return buttons;
}
...
void render_lobby(int width, int height, const std::vector<ButtonDesc>& buttons) {
    ...
    SDL_Rect mods_panel = to_pixels(layout_rect("mods_panel", kModsPanelFallback), width, height);
