...
SettingMetadata make_slider_setting(SettingScope scope, const char* key, const char* label,
                                    const char* description,
                                    std::initializer_list<const char*> categories, float min,
                                    float max, float step, float default_value) {
...
SettingMetadata make_toggle_setting(SettingScope scope, const char* key, const char* label,
                                    const char* description,
...
SettingMetadata make_option_setting(SettingScope scope, const char* key, const char* label,
                                    const char* description,
                                    std::initializer_list<const char*> categories,
                                    const std::vector<SettingOption>& options,
...
SettingMetadata make_option_setting(SettingScope scope, const char* key, const char* label,
                                    ...
                                    const char* default_value) {
    return make_option_setting(scope, key, label, description, categories,
                               std::vector<SettingOption>(options.begin(), options.end()),
                               default_value);
}

std::vector<SettingOption> video_resolution_options() {
    return {
        // 16:9 Desktop / TV
...
void register_engine_settings_schema_entries(EngineState& engine) {
    ...
    // Localization
    schema.add_setting(
        make_option_setting(SettingScope::Install, "gubsy.localization.language", "Language",
                            "Sets the UI language.", {"Profiles"},
                            {SettingOption{"en", "English"}, SettingOption{"es", "Spanish"},
                             SettingOption{"fr", "French"}},
                            "en"));

    // Audio
    {
        SettingMetadata meta =
            make_slider_setting(SettingScope::Install, "gubsy.audio.master_volume", "Master Volume",
                                "Controls the overall output volume for engine-managed audio.",
                                {"Audio"}, 0.0f, 1.0f, 0.01f, 1.0f);
        meta.order = -100;
        schema.add_setting(meta);
    }
    {
        SettingMetadata meta = make_slider_setting(
            SettingScope::Install, "gubsy.audio.music_volume", "Music Volume",
            "Controls the music mix level.", {"Audio"}, 0.0f, 1.0f, 0.01f, 0.8f);
        meta.order = -99;
        schema.add_setting(meta);
    }
    {
        SettingMetadata meta = make_slider_setting(
            SettingScope::Install, "gubsy.audio.sfx_volume", "SFX Volume",
            "Controls the volume for sound effects.", {"Audio"}, 0.0f, 1.0f, 0.01f, 1.0f);
        meta.order = -98;
        schema.add_setting(meta);
    }
    schema.add_setting(make_slider_setting(SettingScope::Install, "gubsy.audio.ui_volume",
                                           "UI Volume", "Adjusts menu and interface sound effects.",
                                           {"Audio"}, 0.0f, 1.0f, 0.01f, 0.75f));
    schema.add_setting(make_option_setting(
        SettingScope::Install, "gubsy.audio.output_device", "Output Device",
        "Selects the preferred audio output device.", {"Audio"},
        {SettingOption{"default", "System Default"}, SettingOption{"headset", "Headset"},
    ...
    {
        SettingMetadata meta = make_slider_setting(
            SettingScope::Profile, "gubsy.input.mouse_sensitivity", "Mouse Sensitivity",
            "Scales mouse look speed.", {"Controls"}, 0.1f, 10.0f, 0.1f, 1.0f);
        meta.order = -100;
        schema.add_setting(meta);
    }
    schema.add_setting(make_slider_setting(SettingScope::Profile, "gubsy.input.vibration_strength",
                                           "Vibration Strength", "Scale controller rumble output.",
                                           {"Controls"}, 0.0f, 1.0f, 0.05f, 0.8f));
    schema.add_setting(make_toggle_setting(SettingScope::Profile, "gubsy.input.invert_y_axis",
                                           "Invert Y-Axis", "Flip vertical look direction.",
                                           {"Controls"}, false));
