...
constexpr WidgetId kSearchWidgetId = 1502;
constexpr WidgetId kPageLabelWidgetId = 1503;
constexpr WidgetId kPrevButtonId = 1504;
constexpr WidgetId kNextButtonId = 1505;
constexpr WidgetId kBackButtonId = 1530;
...
void rebuild_filter(BindsProfilesState& st, const std::vector<BindsProfile>& profiles) {
...
void command_create_profile(MenuContext& ctx, std::int32_t) {
    ...
    auto make_name = [&pool]() {
        int counter = 1;
        while (counter < 1000) {
            std::string candidate = "Binds " + std::to_string(counter);
            bool exists = false;
    ...
    open_binds_editor(ctx, profile.id);
}
...
BuiltScreen build_binds_profiles(MenuContext& ctx) {
    ...
    widgets.push_back(make_label_widget(kTitleWidgetId, SettingsObjectID::TITLE, "Binds Profiles"));
    ...
    search.placeholder = "Search binds profiles...";
    ...
    for (int i = 0; i < kProfilesPerPage; ++i) {
        ...
        if (filtered_idx < static_cast<int>(st.filtered_indices.size())) {
            ...
            if (profile_index == kNewProfileMarker) {
                card.label = "New Binds Profile";
                card.secondary = "Create a new binds profile.";
                ...
                const BindsProfile& profile = profiles[static_cast<std::size_t>(profile_index)];
